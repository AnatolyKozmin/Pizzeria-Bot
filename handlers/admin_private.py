...
async def admin_features(message: types.Message, session: AsyncSession):
    ...
    await message.answer("Выберите вид", reply_markup=get_callback_btns(btns=btns))
...
# Становимся в состояние ожидания ввода name
@admin_router.message(StateFilter(None), F.text == "Добавить меро")
async def add_product(message: types.Message, state: FSMContext):
    await message.answer(
        "Введите название меро", reply_markup=types.ReplyKeyboardRemove()
    )
    await state.set_state(AddProduct.name)
...
async def add_name(message: types.Message, state: FSMContext):
    ...
    await message.answer("Введите описание меро")
...
async def category_choice(callback: types.CallbackQuery, state: FSMContext , session: AsyncSession):
    if int(callback.data) in [category.id for category in await orm_get_categories(session)]:
        ...
        await callback.message.answer('Теперь введите категорию.')
...
async def add_price(message: types.Message, state: FSMContext):
    if message.text == "." and AddProduct.product_for_change:
        ...
        try:
            ...
            await message.answer("Введите корректное значение человек")
            return

        await state.update_data(price=message.text)
    await message.answer("Загрузите изображение меропрития")
...
async def add_image(message: types.Message, state: FSMContext, session: AsyncSession):
    ...
    try:
        ...
        await message.answer("меро добавлено/изменено", reply_markup=ADMIN_KB)
        await state.clear()
...
async def add_image2(message: types.Message, state: FSMContext):
    await message.answer("Отправьте фото меро")